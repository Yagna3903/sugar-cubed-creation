...
export default async function ProductsListPage({ searchParams }: PageProps) {
  ...
  const status: "all" | "active" | "archived" =
    statusParam === "all" || statusParam === "archived" ? (statusParam as any) : "active";
    ...
    <div>
      ...
      <div className="mb-4 mt-3 flex items-center justify-between gap-3">
        ...
        <nav className="flex items-center gap-2" aria-label="Filter products">
          <Filter href="/admin/products?status=all" label="All" active={status === "all"} />
          <Filter href="/admin/products?status=active" label="Active" active={status === "active"} />
          <Filter href="/admin/products?status=archived" label="Archived" active={status === "archived"} />
        </nav>

        <Link href="/admin/products/new" className="rounded-xl bg-brand-brown px-4 py-2 text-white">
          New product
        </Link>
      </div>
      ...
      {products.length === 0 ? (
        <p className="text-zinc-600">
          {status === "active" ? "No active products." : status === "archived" ? "No archived products." : "No products yet."}
        </p>
      ) : (
        <div className="divide-y rounded-2xl border bg-white">
          {products.map((p: ProductRow) => (
            <div key={p.id} className="grid grid-cols-12 items-center gap-4 p-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={[
                        ...
                        p.active ? "bg-green-100 text-green-800" : "bg-zinc-100 text-zinc-700",
                      ].join(" ")}
                    >
              ...
              <div className="col-span-2 text-sm">${(p.priceCents / 100).toFixed(2)}</div>
              <div className="col-span-2 text-sm">{p.inventory?.stock ?? 0} in stock</div>

              <div className="col-span-2 flex justify-end gap-2">
                <Link className="rounded-xl border px-3 py-1.5 text-sm" href={`/admin/products/${p.id}`}>
                  Edit
                </Link>
